...
namespace TNov
{
    ...
    public class failedrooms : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            string TNovClassName = "Лишние удалить"; DateTime dateTime = DateTime.Now;
            ...
            {
                var viewModel = new qwindow280ViewModel();
                viewModel.headtxt = "В проекте присутствуют неразмещенные или избыточные помещения в количестве " + ec + " шт. Удалить их?";
                var wpfview = new qwindow280(viewModel);
                viewModel.CloseRequest += (s, e) => wpfview.Close();
                bool? ok = wpfview.ShowDialog();
                if (ok != null && ok == true) { runIt = true; } else { return Result.Cancelled; }
            }
            else
            {
