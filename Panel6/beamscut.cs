...
namespace TNov
{
    ...
    public class beamscut : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            string TNovClassName = "Перемычки"; DateTime dateTime = DateTime.Now;
            ...
            string N_Par_mrk = "A_Марка изделия";
            if (oldProject == true) { N_Par_mrk = "Мрк.МаркаИзделия"; }
            string N_Par_main = "A_Главная деталь изделия";
            if (oldProject == true) { N_Par_main = "Орг.ГлавнаяДетальСборки"; }
            ...
            foreach (var beam in beams) //ищем перемычки брус
            {
                ...
                Element elem = doc.GetElement(beam.Id); bool paramMrkExist = param.ParamExist(N_Par_mrk, elem);
                if (paramMrkExist)
                {
                    string beamMarkValue = beam.LookupParameter(N_Par_mrk).AsString();
                    if (beamMarkValue != null)
                    {
                        if (beamMarkValue.Contains("ПР"))
                        {
                            concreteBeams.Add(beam);
                            bool paramMainExist = param.ParamExist(N_Par_main, elem);
                            if (paramMainExist)
                            {
                                int beamMainValue = beam.LookupParameter(N_Par_main).AsInteger();
            ...
            foreach (var rebar in rebar2) //ищем перемычки мет
            {
                ...
                Element elem = doc.GetElement(rebar.Id); bool paramMrkExist = param.ParamExist(N_Par_mrk, elem);
                if (paramMrkExist)
                {
                    string beamMarkValue = rebar.LookupParameter(N_Par_mrk).AsString();
                    if (beamMarkValue != null)
                    {
                        if (beamMarkValue.Contains("ПР"))
                        {
                            ...
                            if (paramMainExist)
                            {
                                int beamMainValue = rebar.LookupParameter(N_Par_main).AsInteger();
            ...
            if (viewModel.cut)
            {
                int allcount = concreteBeams.Count;
                ...
                using (Transaction transaction = new Transaction(doc))
                {
                    ...
                    foreach (FamilyInstance beam in concreteBeams)
                    {
                        PBCount++;
