...
namespace TNov
{
    ...
    public class CTpartitiondelete : IExternalCommand
    {
        ...
        {
            string TNovClassName = "Удалить перегородки"; DateTime dateTime = DateTime.Now;
            ...
            List<FamilySymbol> partitiontypes = new List<FamilySymbol>(); //список типов крышек в проекте
            ...
            {
                new infowindow280("Ошибка! В проекте отсутствуют загруженные семейства перегородок.").ShowDialog();
            ...
            CTList = CTpartitiondelete.GetCableTraysFromCurrentSelection(doc, selection); //получаем лотки из текущей выборки
            ...
            using (Transaction transaction = new Transaction(doc))
            {
                ...
                transaction.Start("TNov - Удалить перегородки лотков");
                ...
                Logger.Log("Удаляем перегородки", 1);
                ICollection<ElementId> partitionstoremove = new List<ElementId>();
                
                
                foreach (FamilyInstance partition in partitions1)
                {
                    ...
                    if (mrkstr != null)
                    {
                        ...
                        if (mrkint != 0)
                        {
                            foreach (CableTray ct in CTList1)
                            {
                                ...
                                if (ctid == mrkint) { partitionstoremove.Add(partition.Id); count++; break; }
                            }
                        }
                    }
                }
                if (partitionstoremove.Count > 0) { doc.Delete(partitionstoremove.ToArray()); Logger.Log("Удалено " + count.ToString() + " элементов", 1); }
                else { Logger.Log("перегородки отсутствуют", 1); }
                ...
                if (count > 0)
                {
                    if (count == 1) { var info1 = new infowindow280("Успешно!\nПерегородка лотка удалена."); info1.ShowDialog(); }
                    else { var info1 = new infowindow280("Успешно!\nУдалены перегородки в количестве " + count.ToString() + " шт."); info1.ShowDialog(); }
                }
