...
{
    /// <summary>
    /// Логика взаимодействия для adskgwpf.xaml
    /// </summary>
    public partial class adskgwpf : Window
    {
        public adskgwpf(adskgViewModel viewModel)
        {
            InitializeComponent();
        ...
        private void settings_Click(object sender, RoutedEventArgs e)
        {
            ...
            adskpViewModel viewModel = new adskpViewModel();
            ...
            if (canserialize)
            {
                viewModel = JsonConvert.DeserializeObject<adskpViewModel>(File.ReadAllText(jsonpath));
            }
            else
            {
                //базовые значения
                new adskg().adskpvalues(type, tagParts[0], viewModel);
            }
            var wpfview = new adskpwpf(viewModel);
