...
namespace TNov
{
    ...
    public class IdSelectionTasks : IExternalCommand
    {
        
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            string TNovClassName = "Задания Поиск по номеру"; DateTime dateTime = DateTime.Now;
            ...
            var wpfview = new IdSelectionTasksWPF(viewModel);
            ...
            using (Transaction trans1 = new Transaction(doc))
            {
                ...
                RevitAPI.UiDocument.ActiveGraphicalView.DisableTemporaryViewMode(TemporaryViewMode.TemporaryHideIsolate);
            ...
            List<FamilyInstance> GMs = new FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_GenericModel)   //фильтр по категории Об модели
                                                                         ...
                                                                         .Cast<FamilyInstance>()
                                                                         .ToList();
            List<ElementId> GMIds = new List<ElementId>();
            string[] marks = ids.Split(',');
            foreach (string mark in marks)
            {
                foreach(var GM in GMs)
                {
                    if (GM.get_Parameter(BuiltInParameter.ALL_MODEL_MARK).HasValue)
                    {
                        if(GM.get_Parameter(BuiltInParameter.ALL_MODEL_MARK).AsString()==mark) GMIds.Add(GM.Id);
                    }
                }
            }
            if( GMIds.Count == 0)
            ...
            if (runIt)
            {
                using (Transaction trans2 = new Transaction(doc))
                {
                    if (isolate && cutview && viewtype.Contains("3D"))
                    {
                        ...
                        RevitAPI.UiDocument.ActiveGraphicalView.IsolateElementsTemporary(GMIds.ToArray());
